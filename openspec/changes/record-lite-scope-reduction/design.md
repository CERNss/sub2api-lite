...
| R15 | 低 ✅ | 指向 `main` 分支的链接全是死链（本 fork 只有 `develop-lite`）：合规文档 `blob/main`（Go 常量 + 前端 fallback），以及 README / README_JA / deploy/README 里的 `raw.githubusercontent .../main/deploy/*.sh` 安装命令 —— 用户照抄会得到 `bash: 404:: command not found`（实测踩中）。已修复：15 处全部改为 `develop-lite` |
