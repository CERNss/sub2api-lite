...
import { buildExternalMenuUrl } from '@/utils/external-menu-url'
...
function navItemFromCustomMenu(item: { id: string; label: string; icon_svg: string; url: string; open_mode?: string }): NavItem {
  const externalUrl = item.open_mode === 'external' ? item.url : undefined
  return {
    path: externalUrl ? `external:${item.id}` : `/custom/${item.id}`,
    label: item.label,
    icon: null,
...
// develop-lite 精简模式：Backend 模式下隐藏"面向用户 / 经营性"菜单
//（渠道管理、订阅、公告、兑换码、优惠码等）——纯 API 服务器用不到这些入口。
// 返回 false 即隐藏；backend_mode_enabled 未加载（undefined）时也按隐藏处理，
// 避免菜单先闪现再消失（本分支默认 backend 模式为开）。
const flagHideInBackendMode = (): boolean =>
  appStore.cachedPublicSettings?.backend_mode_enabled === false
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/subscriptions', label: t('nav.mySubscriptions'), icon: CreditCardIcon, hideInSimpleMode: true, featureFlag: flagHideInBackendMode },
    { path: '/profile', label: t('nav.profile'), icon: UserIcon },
    ...customMenuItemsForUser.value.map(navItemFromCustomMenu),
  )
  return items
}
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/groups', label: t('nav.groups'), icon: FolderIcon, hideInSimpleMode: true },
    {
      ...
      expandOnly: true,
      featureFlag: flagHideInBackendMode,
      children: [
        { path: '/admin/channels/pricing', label: t('nav.channelPricing'), icon: PriceTagIcon },
        { path: '/admin/channels/monitor', label: t('nav.channelMonitor'), icon: SignalIcon, featureFlag: flagChannelMonitor },
      ],
    },
    { path: '/admin/subscriptions', label: t('nav.subscriptions'), icon: CreditCardIcon, hideInSimpleMode: true, featureFlag: flagHideInBackendMode },
  ...
  if (authStore.isSimpleMode) {
    ...
    for (const cm of customMenuItemsForAdmin.value) {
      filtered.push(navItemFromCustomMenu(cm))
    }
    return filtered
  }
  ...
  for (const cm of customMenuItemsForAdmin.value) {
    visible.push(navItemFromCustomMenu(cm))
  }
  return visible
})
...
function handleMenuItemClick(item: string | NavItem, event?: MouseEvent) {
  ...
  if (typeof item !== 'string' && item.externalUrl) {
    event?.preventDefault()
    const url = buildExternalMenuUrl(item.externalUrl, authStore.token)
